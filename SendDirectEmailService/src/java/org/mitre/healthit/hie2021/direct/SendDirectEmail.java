...
public class SendDirectEmail {
     ...
     * @param pToEmail
     */
    public static void sendEncryptedAndSignedEmailFromSyntheticMass(String pSubject, String pBody, String pToEmail, InputStream pToCertificate) {
        ...
        sendEncryptedAndSignedEmail(session, fromEmail, fromName, pToEmail, pSubject, pBody, pToCertificate);
    }
        
    public static void sendEncryptedAndSignedEmail(Session session, String fromEmail, String fromName, String toEmail, String subject, String body, InputStream pToCertificate) {
        ...
        MimeMessage encryptedMessage = encryptMessage(session, signedMessage, pToCertificate);
    ...
    private static final String KEY_STORE = "/opt/direct/direct_certs/org.p12";
    ...
    private static void loadCrypto() {
        /* Add BC */
        //Security.addProvider(new BouncyCastleProvider());
        // don't need to to do the above Security.addProvider if we do these steps instead:
    ...
    private static MimeMessage encryptMessage(Session session, MimeMessage pMessage, InputStream pToCertificate) {
        try {
            /* load the certificate */
            CertificateFactory cf = CertificateFactory.getInstance("X.509");
            Certificate certForEncrypt = cf.generateCertificate(new BufferedInputStream(pToCertificate));
            ...
            encrypter.addRecipientInfoGenerator(new JceKeyTransRecipientInfoGenerator((X509Certificate)certForEncrypt).setProvider("BC"));
    ...
    /** original sendEmail with encryptMessage
    public static void sendEmail(Session session, String fromEmail, String fromName, String toEmail, String subject, String body, boolean pEncrypt) {
        try {
            MimeMessage msg = new MimeMessage(session);
            msg.addHeader("Content-type", "text/HTML; charset=UTF-8");
            msg.addHeader("format", "flowed");
            ...
            msg.setRecipients(Message.RecipientType.TO, InternetAddress.parse(toEmail, false));
            if (pEncrypt)
                encryptMessage(msg, subject, body);
            System.out.println("Message is ready");
            Transport.send(msg);
            System.out.println("EMail Sent Successfully!!");
        } catch (Exception e) {
    ...
    private static void encryptMessage(MimeMessage pMessage, String pSubject, String pContent) {
        //
        // Open the key store
        //
        try {
            KeyStore    ks = KeyStore.getInstance("PKCS12", "BC");
            ks.load(new FileInputStream(KEY_STORE), KEY_STORE_PASSWORD.toCharArray());
            ...
            pMessage.setSubject(pSubject);
            pMessage.setContent(mp.getContent(), mp.getContentType());
